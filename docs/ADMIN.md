...
| Brute force | Failures counted per IP in `login_attempts` — a table, not an in-memory map, so a restart does not reset an attacker's budget and the count survives multiple Node processes. **Fails open** if the database is unreachable: that query runs before credentials are checked, and letting it throw turned every login into a 500 with no clue why (production, 2026-08-27). Auth integrity is unaffected — the password check reads env, not the database — so an unreadable counter degrades brute-force protection without ever admitting a wrong password. |
